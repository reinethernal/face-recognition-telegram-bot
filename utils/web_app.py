...
def create_web_app(
    ...
    async def add_user(
        ...
        if not validate_web_code(username, code):
            raise HTTPException(
                status_code=403, detail="Недействительный код или нет прав"
            )

        if photo.content_type not in {"image/jpeg", "image/png"}:
            raise HTTPException(
                status_code=400, detail="Поддерживаются только JPEG/PNG"
            )
        ...
        try:
            ...
            if new_encoding is None:
                os.remove(file_path)
                raise HTTPException(
                    status_code=400, detail="Лицо не обнаружено на фото"
                )
            ...
            if os.path.exists(file_path):
                os.remove(file_path)
            raise HTTPException(
                status_code=500, detail="Не удалось сохранить пользователя"
            )
