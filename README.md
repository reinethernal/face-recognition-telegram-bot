...
- **Система прав**: Доступ к функциям ограничен для администраторов (настраивается через `config.yaml` или в боте). Веб-интерфейс защищён авторизацией по Telegram-username и одноразовому коду из бота.
...
2. **Установите зависимости**:
   ...
   > Если при запуске видите `ModuleNotFoundError: No module named 'face_recognition'`, повторно активируйте виртуальное окружение и переустановите зависимости (`source venv/bin/activate && pip install -r requirements.txt`).
...
3. **Настройте `config.yaml`**:
   ...
   - Отредактируйте `config.yaml`:
     ...
     telegram:
       ...
       admin_usernames_path: "admins.yaml"
     streams:
     ...
     - `admin_ids`: Список Telegram ID администраторов (стартовые права).
     - `admin_usernames`: Список Telegram-username администраторов (можно изменить через команду `Admins` или `/admins`).
     - `admin_usernames_path`: Путь к файлу, где будет сохранён список администраторов, чтобы настройки переживали перезапуск.
     - `streams`: Список RTSP-URL ваших видеопотоков.
     - `known_faces_path`: Папка для хранения изображений известных лиц.
     - `yolo`: Настройки детекции лиц через YOLO11 (`model_path`, `confidence`, `stable_frames`).
     - `web`: Параметры веб-интерфейса (`host`, `port`) для добавления пользователей через API.
...
2. **Команды в Telegram**:
   - `/start`: Запускает бота и показывает меню.
    - **Для администраторов**:
      ...
      - `Администраторы` или команда `/admins`: установить список Telegram-username администраторов (username отправителя добавляется автоматически).
      - `Код для веб` или команда `/webcode`: получить одноразовый код для входа в веб-интерфейс.
      - `Плагин: face_counter`: Показывает количество уникальных обнаруженных лиц.

3. **Веб-интерфейс**:
   - Запускается автоматически (по умолчанию `http://0.0.0.0:8000`).
   - POST `/users` с полями `name`, `username`, `code` и файлом `photo` (JPEG/PNG) добавляет или обновляет лицо в базе. `username` должен быть в списке администраторов, `code` — одноразовый код, который выдаёт бот по кнопке "Код для веб".
