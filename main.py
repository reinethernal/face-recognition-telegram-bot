...
def _save_admin_usernames(path: str, usernames: Iterable[str]) -> None:
    ...
    with open(path, "w", encoding="utf-8") as f:
        yaml.safe_dump(
            {"usernames": sorted({normalize_username(u) for u in usernames})},
            f,
            allow_unicode=True,
        )
...
admin_usernames: Set[str] = _load_admin_usernames(
    admin_usernames_path, initial_admin_usernames
)
...
detection_streaks: Dict[str, Dict[str, int]] = {
    stream: {} for stream in config["streams"].keys()
}
...
def start_web_interface():
    ...
    config_uvicorn = uvicorn.Config(
        app=app, host=web_host, port=web_port, log_level="info"
    )
...
def create_streams_keyboard():
    keyboard = [
        [KeyboardButton(text=str(stream))] for stream in config["streams"].keys()
    ]
...
async def prompt_admins_update(message: types.Message, state: FSMContext):
    if not is_admin_user(message.from_user):
        await message.answer(
            "У вас нет прав на управление администраторами.",
...
async def send_web_code(message: types.Message):
    try:
        if not is_admin_user(message.from_user):
            await message.answer(
                "У вас нет прав на получение кода.", reply_markup=create_main_menu()
            )
            return
...
def require_username(user: types.User) -> str:
    if not user.username:
        raise ValueError(
            "У вас не указан username в Telegram. Установите его в настройках и попробуйте снова."
        )
...
def generate_web_code(username: str, ttl_seconds: int = 300) -> str:
    ...
    with web_code_lock:
        web_login_codes[normalize_username(username)] = {
            "code": code,
            "expires": expires,
        }
    return code
...
async def start(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "Добро пожаловать! Обработка потоков уже запущена. Выберите действие:",
...
@dp.message(
    ...
    and (
        message.text
        in ["Начать", "Добавить лицо", "Получить кадр", "Администраторы", "Код для веб"]
...
async def handle_choice(message: types.Message, state: FSMContext):
    ...
    if choice == "Начать":
        await message.answer(
            "Обработка видеопотоков уже запущена автоматически.",
            reply_markup=main_menu_reply_markup,
        )
    elif choice == "Добавить лицо":
        await message.answer(
            "Пожалуйста, отправьте фотографию лица для добавления.",
            reply_markup=main_menu_reply_markup,
        )
        ...
        if plugin_name in plugins and hasattr(plugins[plugin_name], "execute"):
            result = await plugins[plugin_name].execute(
                bot, message, chat_id, detected_faces
            )
            await message.answer(result, reply_markup=main_menu_reply_markup)
        else:
            await message.answer(
                f"Плагин {plugin_name} не найден или не поддерживает выполнение.",
                reply_markup=main_menu_reply_markup,
            )
        await state.clear()
...
@dp.message(
    StateFilter(AddFace.waiting_for_photo),
...
async def receive_photo(message: types.Message, state: FSMContext):
    ...
    if not is_admin_user(message.from_user):
        await message.answer(
            "У вас нет прав на добавление лиц.", reply_markup=main_menu_reply_markup
        )
        await state.clear()
        return

    temp_file_path = os.path.join(
        known_faces_path, f"temp_{message.from_user.id}_{message.message_id}.jpg"
    )
    ...
    try:
        ...
        await message.answer(
            f"Фотография временно сохранена как {temp_file_path}",
            reply_markup=main_menu_reply_markup,
        )
        ...
        if new_face_encoding is not None:
            ...
            await message.answer(
                "Лицо распознано. Укажите имя для этого лица.",
                reply_markup=main_menu_reply_markup,
            )
            ...
            await message.answer(
                "Лицо не обнаружено на фотографии.", reply_markup=main_menu_reply_markup
            )
        ...
        await message.answer(
            f"Ошибка при обработке фото: {str(e)}", reply_markup=main_menu_reply_markup
        )
...
@dp.message(
    StateFilter(AddFace.waiting_for_name), lambda message: message.text is not None
)
async def receive_name(message: types.Message, state: FSMContext):
    ...
    if not temp_file_path or not os.path.exists(temp_file_path):
        ...
        await message.answer(
            "Ошибка: временный файл не найден.", reply_markup=main_menu_reply_markup
        )
        await state.clear()
        return
    ...
    try:
        ...
        if new_face_encoding is not None:
            ...
            await message.answer(
                f"Лицо '{name}' успешно добавлено!", reply_markup=main_menu_reply_markup
            )
        else:
            logger.error(f"Не удалось распознать лицо при сохранении для '{name}'")
            await message.answer(
                f"Ошибка: не удалось распознать лицо для '{name}'.",
                reply_markup=main_menu_reply_markup,
            )
            os.remove(file_path)
    except Exception as e:
        ...
        await message.answer(
            f"Ошибка при сохранении лица: {str(e)}", reply_markup=main_menu_reply_markup
        )
        if os.path.exists(temp_file_path):
            os.remove(temp_file_path)
...
@dp.message(
    StateFilter(AdminManagement.waiting_for_admins),
...
async def update_admins(message: types.Message, state: FSMContext):
    ...
    if not is_admin_user(message.from_user):
        await message.answer(
            "У вас нет прав на изменение списка администраторов.",
            reply_markup=main_menu_reply_markup,
        )
        await state.clear()
        return
...
async def select_stream(message: types.Message, state: FSMContext):
    ...
    if not is_admin_user(message.from_user):
        await message.answer(
            "У вас нет прав на получение кадров.", reply_markup=main_menu_reply_markup
        )
        await state.clear()
        return
    ...
    if message.text == "Отмена":
        ...
        await message.answer(
            f"Ожидаю следующий кадр из потока {stream_name}...",
            reply_markup=main_menu_reply_markup,
        )
        await state.clear()
    else:
        await message.answer(
            "Неверный выбор. Пожалуйста, выберите поток из списка.",
            reply_markup=create_streams_keyboard(),
        )
        ...
        while True:
            ...
            try:
                ...
                for name in current_faces:
                    ...
                    detection_streaks[stream_name][name] = (
                        detection_streaks[stream_name].get(name, 0) + 1
                    )

                for name in list(last_seen_time[stream_name].keys()):
                    if (
                        name not in current_faces
                        and (current_time - last_seen_time[stream_name][name])
                        >= RESET_TIMEOUT
                    ):
                ...
                if face_locations:
                    logger.info(
                        f"Обнаружено {len(face_locations)} лиц в потоке {stream_name}"
                    )
                    ...
                    for name in face_names:
                        if (
                            detection_streaks[stream_name].get(name, 0)
                            < stable_frame_count
                        ):
                            continue

                        if (
                            name not in detected_faces[stream_name]
                        ):  # Первое появление после подтверждения
                            detected_faces[stream_name].add(name)
                            if name != "Unknown":
                                logger.info(
                                    f"Обнаружено лицо: {name} в потоке: {stream_name}"
                                )
                                message = (
                                    f"Обнаружено лицо: {name} в потоке: {stream_name}"
                                )
                                ...
                                await bot.send_message(
                                    chat_id,
                                    f"Обнаружено неизвестное лицо в потоке: {stream_name}",
                                )
                                photo = FSInputFile(last_frame_path)
                                await bot.send_photo(chat_id=chat_id, photo=photo)
                else:
...
async def main():
    ...
    for stream_name, stream_url in config["streams"].items():
        task = asyncio.create_task(
            process_stream(stream_name, stream_url, task_manager)
        )
